...
它们不是后来重新拼出来的示例，而是从同一个 ChatGPT Sites 私有源仓库的真实历史提交中导出的两个时间切片。公开时只移除了私有 Sites 项目绑定。

### v1 · `bb0ce0d` · Build interactive carrier launch 3D demo

[打开 v1 源码](versions/v1-initial/)

v1 是第一次把“舰载机弹射”变成可交互网页的版本。它采用典型的快速 Vibe Coding 策略：把航母、飞机、海面、镜头、UI 和动画集中在一个 `app/page.tsx` 中，用代码几何体快速验证故事是否成立。
