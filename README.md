...
| 阶段 | 调用方式 | 产物 | 结论 |
|:--:|:--|:--|:--|
| **01** | **LLM + Prompt** | One-shot 舰载机弹射 GIF | 能表达主题，缺少统一空间与工程约束 |
| **02** | **LLM + Prompt** | 代码生成的 2D 弹射 GIF | 动作更明确，仍不是可验证的 3D 系统 |
| **03** | **Agent + Prompt** | 可交互 v1 | 页面能运行，但模型与比例依赖 Agent 猜测 |
| **04** | **Agent + Prompt** | 组件化 v2 | 工程结构改善，项目数据和视觉仍未闭环 |
| **05** | **Harness** | 最终私有 Demo + 公开接口 | 输入、执行、验证、人工审核和发布形成闭环 |

---

## 01–02 · LLM + Prompt：两个快速生成结果

| 01 · 多模态 LLM + Prompt | 02 · 代码 LLM + Prompt |
|:--:|:--:|
| [![最初 One-shot GIF](media/01-one-shot-carrier-launch.gif)](media/01-one-shot-carrier-launch.mp4) | ![早期 2D 弹射动画](media/02-code-llm-catapult.gif) |
| [GIF](media/01-one-shot-carrier-launch.gif) · [MP4](media/01-one-shot-carrier-launch.mp4) | [GIF](media/02-code-llm-catapult.gif) |
...
```text
                 ┌────────────── 通过 ──────────────▶ [结束 / 发布]
                 │
[WGY 审核] ── 不通过 ──▶ [LLM 整理 Prompt] ──▶ [Codex 执行] ──▶ [Demo]
    ▲                                      │                 │
    └──────── Harness 记录差异、验证与回归 ┴── 新版本返回审核 ┘
```

循环的停止条件不由 Agent 自己宣布，而由人看过真实运行结果后决定。**通过就结束；不通过就把新的审查意见重新交给 LLM，进入下一轮。**

| 角色 | 在这个项目中的实际职责 |
|:--|:--|
| **Author / reviewer · WGY** | 提供方向与项目资料；观看每一版；指出“像平板”“云太假”“攀升拐直角”“Mac 发热”等问题；决定取舍并完成最终验收。 |
| **LLM** | 与 WGY 对话，把短反馈扩展成包含目标、限制、禁止项和验收标准的 Prompt。 |
| **Execution Agent · Codex** | 检查现有仓库和权威数据源，完成 Three.js 建模、资产接入、动画、状态机、性能优化、构建测试、公开边界检查和部署。 |
| **Harness** | 保存输入契约、工具与文件边界、对象所有权、自动验证、人工 Gate 和发布隔离；失败时把差异送回下一轮。 |

WGY 不需要手工逐行编写整套 Three.js，但始终拥有项目方向、审查权和停止条件；LLM 与 Agent 是实现工具，不是项目作者。

---

## 03 · Agent + Prompt：v1 第一次变成可交互页面
...
v1 审查阶段，WGY 的反馈集中在场景尺度、飞机辨识度和动画状态。LLM 将反馈整理为组件、状态和视觉要求，再由 Codex 继续实现下一版。

---

## 04 · Agent + Prompt：v2 结构改善但视觉仍然不足
...
### 复用为独立仓库：sortie3d-harness

从 v2 暴露的问题中抽取出的数据契约、物理检查、视觉 QA 和迭代 Gate，已经整理成独立项目 [**sortie3d-harness**](https://github.com/wgy577/sortie3d-harness)。它把二维甲板几何和起飞时序导出为 `choreography.json`，执行 Agent 只读取统一契约生成 Three.js 场景，自动检查与多模态 QA 给出可追踪结果，最终仍由 WGY 决定继续迭代或结束。

[![sortie3d-harness 演示](https://github.com/wgy577/sortie3d-harness/raw/main/media/demo-run010.gif)](https://github.com/wgy577/sortie3d-harness)

> **体验提示：** 点击上方 GIF 进入独立 Harness 仓库，查看 SOP、数据契约、物理检查、QA checklist 和迭代记录。

| 维度 | 本案例仓库 | sortie3d-harness |
|:--|:--|:--|
| 用途 | 记录从失败 Prompt 到最终 Demo 的演进 | 把可复用控制机制提取成独立执行框架 |
| 输入 | 项目甲板、MATLAB 轨迹、阶段 Prompt | `choreography.json` 与 `deck_geometry.json` |
| 输出 | v1/v2 证据、最终 Demo 入口与 API | 可重复生成的场景、录制结果与 QA 报告 |
| 验收 | Build/Test + 视觉审查 + 人工 Gate | 物理检查 + 多模态 QA + 人工 Gate |

▶ [**进入 sortie3d-harness**](https://github.com/wgy577/sortie3d-harness) · [查看演示 GIF](https://github.com/wgy577/sortie3d-harness/raw/main/media/demo-run010.gif)

---

## 05 · Harness：不再追加万能 Prompt

从这里开始，要求不再只是“画面应该更好”，而是逐步加入来源、协议、对象、测试和发布门禁。Codex 每次实现都必须在这些约束下检查现有仓库，而不是重新猜一个场景。
...
- LangGraph 的 Human-in-the-loop 使用持久化状态、`interrupt` 和 `resume`：执行到审核点就暂停，保存当前状态，得到人的批准、修改或自然语言反馈后再恢复。这正对应本项目“人说行才结束，否则回到 LLM”的循环。[LangGraph Interrupts](https://langchain-ai.github.io/langgraph/how-tos/human_in_the_loop/wait-user-input/)
...
|--:|:--|:--|:--|:--|
| 01 | **验收契约** | LLM 整理 Prompt 时固定写出 `Goal / Context / Constraints / Done when / 禁止项`，把“好看一点”改成镜头占比、对象数量、动作阶段和持续时间。 | 本 README 的 [Prompt 演进档案](#prompt-演进档案) | 验收条件不完整时不进入执行。 |
| 02 | **人工停止条件** | 每轮生成实际 Demo 或截图供 WGY 审核；只有人工确认“通过”才终止循环。 | 最终截图、v1/v2 预览 GIF、连续审查记录 | “不通过”携带差异说明回到 LLM，不允许 Agent 自行宣布完成。 |
| 03 | **Agent 交接格式** | LLM 输出可执行规格；Codex 返回代码、构建结果和可观看产物；Harness 保存每轮差异与回归点。 | README 中的环形协作图与阶段 Prompt | 交付不完整就不进入下一层，避免 Agent 自行补猜。 |
...
在 Harness 阶段，资产必须先通过候选准入条件，再由 Codex 实际搜索、核对和接入。候选资产不只比较“像不像”，还检查远景轮廓、三角面数、许可、透明材质成本、前向轴和贴地难度。
...
| 阶段 | 结果 | Harness 成熟度 |
|:--|:--|:--|
| **01–02 · LLM + Prompt** | 两个 GIF 很快表达主题，但没有可信三维空间 | `L0` Direct Prompt |
| **03 · Agent + Prompt** | v1 页面可交互，但航母和飞机明显粗糙 | `L1` Visual Contract |
| **04 · Agent + Prompt** | v2 组件和状态改善，视觉与项目数据仍然不足 | `L2` Prompt Structure |
| **05 · Harness** | 坐标、舰体、资产、轨迹、对象和状态逐层隔离 | `L3–L4` Engineering Harness |
...
> 这些长 Prompt 的主要生成链是：**WGY 给出短反馈 → LLM 整理 Prompt → Codex 实现 → WGY 验收**。下方为可读性合并了重复路径、上传确认和按钮微调，但保留了所有关键方向变化。

<details>
<summary><strong>01 · LLM + Prompt：先生成弹射 GIF</strong></summary>
...
<details>
<summary><strong>02 · LLM + Prompt：用代码生成 2D 弹射 GIF</strong></summary>

```text
基于项目中的甲板与舰载机语义，用代码生成一段简洁的 2D GIF。
画出甲板、弹射器、舰载机加速、离舰和攀升过程。
优先让路径和动作可读，不追求真实三维空间。
```

动作比第一版清楚，但仍然只是二维动画，不能旋转观察，也无法验证舰体、资产和空间关系。
</details>

<details>
<summary><strong>03 · Agent + Prompt：v1 完整交互原型</strong></summary>
...
<details>
<summary><strong>04 · Agent + Prompt：v2 紧凑场景与状态拆分</strong></summary>
...
原样公开 v1/v2，展示 Agent + Prompt 原型到 Harness 系统的过程。
...
v1/v2 是未经美化的 Agent + Prompt 历史证据。它们的不足不是需要隐藏的瑕疵，而是理解 Harness 为什么必要的对照组。

## 数据、素材与用途声明

本项目中的调度、轨迹与交互演示用于科研仿真和 Agent Engineering 展示，不代表真实装备性能、实际作业流程或安全结论。仓库内原创代码、文档和项目生成素材按下方许可证使用；页面中列明或引用的第三方模型、纹理与环境素材，其版权和许可仍归原作者及来源平台所有，本仓库不授予超出原许可范围的再分发或商业使用权。
...
**Directed and reviewed by WGY · LLM-assisted prompts · Codex execution · Harness-controlled delivery**
